...
const ROLLS_KEY = "rolls:salvage-rights";
...
async function handleRollsPost(request, env) {
  ...
  // setback: 2, ... }) — stored alongside the summarised result so the
  // shared log can render the actual pool as icons, not just the outcome.
  // Sanitized to known die-type keys, non-negative integers, capped at a
  // sane per-die maximum so a malformed/malicious body can't bloat the KV
  // entry.
